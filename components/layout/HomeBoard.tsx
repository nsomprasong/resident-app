...
export function HomeBoard() {
  ...
  const { employee, permissions, loaded } = useEmployeePermissions();

  useEffect(() => {
    if (!loaded) return;
    if (!employee || !hasAllowedMenus(permissions)) {
      router.replace("/access-denied?reason=PERMISSIONS_EMPTY");
    }
  }, [loaded, employee, permissions, router]);
    ...
    <div className="mx-auto max-w-6xl space-y-8 p-4 sm:p-8">
      <div className="relative overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-primary via-primary to-secondary p-7 text-primary-foreground shadow-lg sm:p-10">
        ...
        <div className="relative flex items-start gap-4">
          {/* Native img avoids next/image optimizer on the huge logo.png (~900KB). */}
          ...
          <div className="min-w-0">
            ...
            <p className="mt-3 max-w-xl text-primary-foreground/80">
              สวัสดี {employee.name || "คุณ"} — เมนูถูกจัดกลุ่มตามงานจริง
            </p>
          </div>
        </div>
      </div>
