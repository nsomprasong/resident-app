...
export default function AddBookingResourcesDialog({ open, setOpen, bookingId, checkIn, checkOut, onAdded }: { open: boolean; setOpen: (open: boolean) => void; bookingId: string; checkIn: string; checkOut: string; onAdded: () => void }) {
  ...
  return <Modal open={open} onClose={() => setOpen(false)} title="เพิ่มห้องหรือแพ"><div className="space-y-4"><p className="text-sm text-muted-foreground">เลือกเฉพาะรายการที่ต้องการเพิ่ม สามารถเลือกห้อง แพ หรือทั้งสองอย่างได้</p><ZoneRoomSelect selectedRoomIds={roomIds} onChange={setRoomIds} checkIn={checkIn} checkOut={checkOut} /><RaftSelect selectedRaftIds={raftIds} onChange={setRaftIds} checkIn={checkIn} checkOut={checkOut} />{error && <p className="rounded-xl bg-destructive/10 p-3 text-sm text-destructive">{error}</p>}<button onClick={submit} disabled={saving || (!roomIds.length && !raftIds.length)} className="w-full rounded-xl bg-primary px-4 py-3 font-medium text-primary-foreground disabled:bg-muted-foreground/40">{saving ? "กำลังเพิ่ม..." : "เพิ่มในรายการจอง"}</button></div></Modal>;
}
